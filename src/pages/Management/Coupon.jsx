...
const Coupon = () => {
  ...
  const handleAddCoupon = async () => {
    try {
      await axios.post(
        "https://freezbackend-824bd046e21f.herokuapp.com/api/code/addCode",
        newCouponData
      );
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        "https://freezbackend-824bd046e21f.herokuapp.com/api/code/getAllCode"
      );
  ...
  const confirmDeleteCoupon = async () => {
    try {
      await axios.delete(
        `https://freezbackend-824bd046e21f.herokuapp.com/api/code/deletcode/${couponToDelete}`
      );
      ...
      toast.error(
        "Erreur lors de la suppression du coupon. Veuillez réessayer.",
        {
          position: toast.POSITION.TOP_CENTER,
        }
      );
  ...
  const confirmUpdateCoupon = async (updatedCouponData) => {
    try {
      await axios.put(
        `https://freezbackend-824bd046e21f.herokuapp.com/api/code/updateCode/${couponToUpdate._id}`,
        updatedCouponData
      );
      ...
      toast.error(
        "Erreur lors de la mise à jour du coupon. Veuillez réessayer.",
        {
          position: toast.POSITION.TOP_CENTER,
        }
      );
      ...
      <Dialog open={updateDialogOpen} onClose={cancelUpdateCoupon}>
        ...
        <DialogContent>
          <TextField
            ...
            onChange={(e) =>
              setCouponToUpdate({ ...couponToUpdate, code: e.target.value })
            }
            fullWidth
            margin="normal"
          />
          ...
          <TextField
            ...
            onChange={(e) =>
              setCouponToUpdate({ ...couponToUpdate, dateFin: e.target.value })
            }
            fullWidth
            margin="normal"
          />
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => confirmUpdateCoupon(couponToUpdate)}
            color="primary"
          >
            Mettre à jour
          </Button>
      ...
      <Dialog open={addDialogOpen} onClose={cancelAddCoupon}>
        ...
        <DialogContent>
          ...
          <TextField
            ...
            onChange={(e) =>
              setNewCouponData({ ...newCouponData, dateDebuit: e.target.value })
            }
            fullWidth
            margin="normal"
          />
          ...
          <TextField
            ...
            onChange={(e) =>
              setNewCouponData({ ...newCouponData, dateFin: e.target.value })
            }
            fullWidth
            margin="normal"
          />
