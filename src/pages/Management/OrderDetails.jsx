...
const OrderDetails = () => {
      ...
      try {
        ...
        switch (status) {
          case "Awaiting Confirmation":
            apiUrl = `https://freezbackend-824bd046e21f.herokuapp.com/api/order/updateIsenAttendComfirmation/${orderId}`;
            break;
          case "Available for Printing":
            apiUrl = `https://freezbackend-824bd046e21f.herokuapp.com/api/order/DisponibleInpression/${orderId}`;
            break;
          case "Available for Shipping":
            apiUrl = `https://freezbackend-824bd046e21f.herokuapp.com/api/order/DispoExpedition/${orderId}`;
            break;
          case "In Delivery":
            apiUrl = `https://freezbackend-824bd046e21f.herokuapp.com/api/order/UpdateEncours/${orderId}`;
            break;
          case "Delivered":
            apiUrl = `https://freezbackend-824bd046e21f.herokuapp.com/api/order/updateIsDelivred/${orderId}`;
            break;
          case "Order Refused":
            apiUrl = `https://freezbackend-824bd046e21f.herokuapp.com/api/order/updateIsCanceled/${orderId}`;
            break;
          case "Order Paid":
            apiUrl = `https://freezbackend-824bd046e21f.herokuapp.com/api/order/updateIsPaid/${orderId}`;
            break;
          default:
            break;
        }
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://freezbackend-824bd046e21f.herokuapp.com/api/order/getPanierById/${orderId}`
        );
        ...
        const formattedDate = `${rawDate.getDate()}/${
          rawDate.getMonth() + 1
        }/${rawDate.getFullYear()} ${rawDate.getHours()}:${rawDate.getMinutes()}`;
  ...
  return (
    <div>
      ...
      >
        <StyledButton variant="contained" component={Link} to="/Home">
          Retour
        </StyledButton>
        <div>
          <StyledButton variant="contained" onClick={handleGoToInvoice}>
            Go to Invoice
          </StyledButton>
        </div>
      </div>
      <OrderContainer>
        ...
        <div>
          ...
          >
            ...
            <option value="Available for Printing">
              Disponible pour l'impression
            </option>
            <option value="Available for Shipping">
              Disponible pour l'expédition
            </option>
      ...
      <OrderTableContainer>
        <OrderElements>
          <OrderElement>
            ...
            <StyledElement>
              <OrderTypography>Client</OrderTypography>
          ...
          <OrderElement>
            ...
            {/* ////// Order Info ///// */}
            <StyledElement>
              <OrderTypography>Order Info</OrderTypography>
              <StyledText>
                Methode de paiment: {PanierData.paymentMethod}{" "}
              </StyledText>
              <StyledText>
                Total: {PanierData.prixOfOllOderByUser} {currency}
              </StyledText>
              <StyledText>
                {" "}
                {PanierData.isPaid ? "payé" : "non payé"}{" "}
              </StyledText>
            </StyledElement>
          </OrderElement>
          ...
          <OrderElement>
            ...
            <StyledElement>
              <OrderTypography>Deliver To</OrderTypography>
              <StyledText>
                Address:{" "}
                {PanierData.shippingAddress &&
                PanierData.shippingAddress.length > 0
                  ? PanierData.shippingAddress[0].address1
                  : "non spécifié"}
              </StyledText>

              <StyledText>
                {" "}
                {PanierData.Encours ? "Livraison en cours" : ""}{" "}
              </StyledText>
            </StyledElement>
          </OrderElement>
        </OrderElements>
        ...
        <OrderTable>
          <TableContainer>
            ...
            {PanierData.Orders && PanierData.Orders.length > 0 && (
              <TotalCost>
                <StyledText>
                  Sous-total : {PanierData.prixOfOllOderByUser}{" "}
                  {PanierData.Orders[0].currency}
                </StyledText>
                <StyledText>
                  Frais de livraison : {PanierData.fraisTansport}{" "}
                  {PanierData.Orders[0].currency}
                </StyledText>
                <StyledText>
                  Total général :{" "}
                  {PanierData.prixOfOllOderByUser + PanierData.fraisTansport}{" "}
                  {PanierData.Orders[0].currency}
                </StyledText>
                <StyledText>
                  Statut :{" "}
