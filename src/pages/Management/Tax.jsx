...
const Tax = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        "https://freezbackend-824bd046e21f.herokuapp.com/api/tax/getAllTax"
      );

      if (response.data.success) {
        const countries = response.data.tax.flatMap(
          (taxItem) => taxItem.Country
        );
  ...
  const handleConfirmAdd = async () => {
    try {
      await axios.post(
        "https://freezbackend-824bd046e21f.herokuapp.com/api/tax/addTax",
        {
          Country: [newTaxData],
        }
      );
      ...
      console.error(
        "Erreur lors de l'ajout des informations fiscales :",
        e.message
      );
      toast.error(
        "Erreur lors de l'ajout des informations fiscales. Veuillez réessayer.",
        {
          position: toast.POSITION.TOP_CENTER,
        }
      );
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(
        `https://freezbackend-824bd046e21f.herokuapp.com/api/tax/deletTax/${TaxToDelete}`
      );
      setDeleteDialogOpen(false);
      fetchData();
      ...
      toast.error(
        "Erreur lors de la suppression de la taxe. Veuillez réessayer.",
        {
          position: toast.POSITION.TOP_CENTER,
        }
      );
      ...
      <Dialog open={addDialogOpen} onClose={handleAddDialogClose}>
        ...
        <DialogContent>
          <TextField
            ...
            onChange={(e) =>
              setNewTaxData({ ...newTaxData, city: e.target.value })
            }
            fullWidth
            margin="normal"
          />
          <TextField
            ...
            onChange={(e) =>
              setNewTaxData({ ...newTaxData, shippingProvince: e.target.value })
            }
            fullWidth
            margin="normal"
          />
          <TextField
            ...
            onChange={(e) =>
              setNewTaxData({ ...newTaxData, taxType: e.target.value })
            }
            fullWidth
            margin="normal"
          />
          <TextField
            ...
            onChange={(e) =>
              setNewTaxData({ ...newTaxData, taxRate: e.target.value })
            }
            fullWidth
            margin="normal"
          />
